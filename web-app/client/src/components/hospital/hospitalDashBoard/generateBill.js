...
export default function GenerateBill(props) {
    ...
    function createPatientMeuItems() {
        ...
        for (let i = 0; i < allPatients.length; i++) {
            items.push(<MenuItem
                key={i}
                value={allPatients[i]}>{localStorage.getItem(allPatients[i])}</MenuItem>);
        }
        return items;
    }
    ...
    function createDoctorMenuItems() {
        ...
        for (let i = 0; i < allDoctors.length; i++) {
            items.push(<MenuItem
                key={i}
                value={allDoctors[i]}>{localStorage.getItem(allDoctors[i])}</MenuItem>);
        }
        return items;
    }
    ...
    function createLaboratoryMenuItems() {
        ...
        for (let i = 0; i < allLaboratories.length; i++) {
            items.push(<MenuItem
                key={i}
                value={allLaboratories[i]}>{localStorage.getItem(allLaboratories[i])}</MenuItem>);
        }
        return items;
    }
    ...
    function createPharmacyMenuItems() {
        ...
        for (let i = 0; i < allPharmacies.length; i++) {
            items.push(<MenuItem
                key={i}
                value={allPharmacies[i]}>{allPharmacies[i]}</MenuItem>);
        }
        return items;
    }
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <Typography align='center'>
                                        {selectedBillFile === '' ? chosenFilename : selectedBillFile.name}
                                    </Typography>
                                </Grid>
