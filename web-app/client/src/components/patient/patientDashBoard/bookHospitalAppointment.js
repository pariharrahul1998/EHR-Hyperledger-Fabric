...
export default function BookHospitalAppointment(props) {
    ...
    const submitAppointment = async (event) => {
        ...
        try {
            ...
            appointment.appointmentId = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
            console.log(appointment);
    ...
    function createMenuItems() {
        ...
        for (let i = 0; i < hospitals.length; i++) {
            items.push(<MenuItem
                key={i}
                value={hospitals[i]["hospitalId"]}>{hospitals[i]["hospitalName"] + " " + hospitals[i]["hospitalId"]}</MenuItem>);
                    ...
                    <form style={form} onSubmit={submitAppointment}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} style={{height: 240}}>
                                <TextField rows={10} multiline={true}
